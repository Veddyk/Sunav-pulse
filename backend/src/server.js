...
app.get("/health", async (req, res) => {
  try {
    ...
    // Return 200 (not 503) so Railway's healthcheck passes even when DB is temporarily
    // unreachable. The "degraded" status in the body still signals the real state.
    res.status(200).json({
      status: "degraded",
      db: "unreachable",
      timestamp: new Date().toISOString(),
...
app.post("/debug-auth", async (req, res) => {
  ...
  const result = {
    step1_user_found: false,
    step2_hash_found: false,
    step3_bcrypt_match: false,
    step4_status_active: false,
    user_status: null,
    hash_prefix: null,
    error: null,
  };
  try {
    ...
    result.user_status = row.status;
    result.step4_status_active = row.status === "active";

    // Step 2: hash found
    if (!row.password_hash) {
      result.error = "No password hash in DB";
      return res.json(result);
    }
    result.step2_hash_found = true;
    result.hash_prefix = row.password_hash.slice(0, 10);

    // Step 3: bcrypt compare
    const match = await bcrypt.compare(password, row.password_hash);
    result.step3_bcrypt_match = match;
